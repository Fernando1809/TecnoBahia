...
export default function App() {
  ...
  // Plantillas integradas: el usuario no las sube.
  useEffect(() => {
    let vivo = true;
    (async () => {
      try {
          ...
          <section>
            ...
            <div className="mt-6 rounded-lg border border-border p-4">
              <h3 className="mb-3 text-sm font-medium">
                Promociones ingresadas erróneamente
              </h3>
              {resultado.indicadores.detallePromocionesErroneas.length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  Todas las promociones se pudieron resolver correctamente.
                </p>
              ) : (
                <div className="max-h-60 overflow-auto">
                  <table className="min-w-full text-xs">
                    <thead className="sticky top-0 bg-muted">
                      <tr>
                        {["Documento", "SKU", "Ítem", "Cantidad", "Motivo"].map((h) => (
                          <th
                            key={h}
                    ...
                    <tbody>
                      {resultado.indicadores.detallePromocionesErroneas.map((p, i) => (
                        <tr key={i}>
                          <td className="border-b border-border/60 px-2 py-1.5">{p.documento}</td>
                          <td className="border-b border-border/60 px-2 py-1.5">{p.sku}</td>
                          <td className="border-b border-border/60 px-2 py-1.5">{p.item}</td>
                          <td className="border-b border-border/60 px-2 py-1.5 tabular-nums">
                            {p.cantidad}
                          </td>
                          <td className="border-b border-border/60 px-2 py-1.5">{p.motivo}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
